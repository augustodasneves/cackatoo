...
if(categoria = '')then
  begin
    if Application.MessageBox('Informe a Categoria por Favor','Atenção',+MB_OK+MB_ICONERROR)=1 then
      begin
        edtcategoria.SetFocus;
        dm.tbcategoria.Append;
      end;
...
begin
  ...
  Application.MessageBox('Problemas ao alterar Categoria','Atenção',mb_ok);
  end;
end;

procedure Tfrmcadcategoria.btnexcluirClick(Sender: TObject);
begin
  if (application.MessageBox('Você tem certeza que deseja excluir esta categoria?','Atenção',mb_Iconquestion+Mb_yesno)=idyes)then
    begin
    try
...
begin
  ...
  if (dm.tbcategoria.RecordCount>0)then
    begin
      for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
       if (Components[i]is Tedit)then  //verifica se os componentes são dbedits
       TDBEdit(Components[i]).Enabled:=false;
       tratabotao(3);
  end
  else
    begin
      Application.MessageBox('Não há nenhum registro de categoria','Atenção',mb_ok+MB_ICONEXCLAMATION);
      for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
