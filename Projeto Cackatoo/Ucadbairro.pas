...
begin
  ...
  Application.MessageBox('Problemas ao alterar Bairro','Atenção',mb_ok);
  end;
end;

procedure Tfrmcadbairro.btnexcluirClick(Sender: TObject);
begin
  if (application.MessageBox('Você tem certeza que deseja excluir este Bairro?','Atenção',mb_Iconquestion+Mb_yesno)=idyes)then
    begin
    try
      dm.tbbairro.Delete;
      ShowMessage('Bairro Excluído com Sucesso!');
      if (dm.tbbairro.RecordCount=0)then
        begin
          tratabotao(1);
        end
...
begin
    ...
    begin
      Application.MessageBox('Você não tem nenhum registro de bairros','Atenção',MB_OK+MB_ICONEXCLAMATION);
      for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
        if ((Components[i]is Tdbedit)or (Components[i] is TSpeedButton)or (Components[i] is Tdbradiogroup)or(Components[i] is TDBComboBox)or(Components[i] is TDBLookupComboBox)or(Components[i] is TDBCheckBox))then  //verifica se os componentes são dbedits
