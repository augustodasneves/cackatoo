...
begin
  if(edt_data_final.Text < edt_data_inicial.Text)then
    ...
    begin
      ShowMessage('A Data final não pode ficar em branco!');
    end
  else if(edt_data_inicial.Text = '')then
    begin
      ShowMessage('A Data inicial não pode ficar em branco!');
    end
  else
...
begin
  ...
  for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
   if ((Components[i]is TDBedit)or(Components[i] is TDBComboBox)or(Components[i] is TDBLookupComboBox)or(Components[i] is TDBCheckBox)or(Components[i] is TEdit))then  //verifica se os componentes são dbedits
...
begin
   try
        ...
        DBGrid1.Enabled:=True;
        Application.MessageBox('Problemas ao cadastrar Rendimento Mensal','Atenção',mb_ok);
    for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
   if ((Components[i]is TDBedit)or(Components[i] is TDBComboBox)or(Components[i] is TDBLookupComboBox)or(Components[i] is TDBCheckBox)or(Components[i] is TEdit))then  //verifica se os componentes são dbedits
    TDBEdit(Components[i]).Enabled:=false;
...
begin
  ...
  Application.MessageBox('Problemas ao alterar Faturamento','Atenção',mb_ok);
  end;
end;

procedure Tfrmfaturamento.btnexcluirClick(Sender: TObject);
begin
   if (application.MessageBox('Você tem certeza que deseja excluir este registro de faturamento?','Atenção',mb_Iconquestion+Mb_yesno)=idyes)then
    begin
    try
      dm.tbrenda_mensal.Delete;
      ShowMessage('Registro de Faturamento Excluído com Sucesso!');
      if (dm.tbrenda_mensal.RecordCount=0)then
        begin
          tratabotao(1);
        end
   ...
   try
    begin
      ...
      if(dm.tbrenda_mensal.recordcount>=1)then
        begin
          for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
            if ((Components[i]is Tdbedit)or (Components[i] is Tdbradiogroup)or(Components[i] is TDBComboBox)or(Components[i] is TDBCheckBox)or (Components[i] is TDBLookupComboBox)or(Components[i] is TEdit))then  //verifica se os componentes são dbedits
              TDBEdit(Components[i]).Enabled:=false;
              tratabotao(3)
            end
          else
            begin
              for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
                if ((Components[i]is Tdbedit)or (Components[i] is Tdbradiogroup)or(Components[i] is TDBComboBox)or(Components[i] is TDBLookupComboBox)or(Components[i] is TDBCheckBox)or(Components[i] is TEdit))then  //verifica se os componentes são dbedits
                  TDBEdit(Components[i]).Enabled:=false;
                  tratabotao(1);
                end;
            end
...
begin
   dm.tbrenda_mensal.Active:=true;
    ...
    begin
      Application.MessageBox('Você não tem nenhum registro de Faturamento','Atenção',MB_OK+MB_ICONEXCLAMATION);
      for i:=0 to componentcount-1 do //passa pelos componentes verificando pelos números
        if ((Components[i]is Tdbedit)or (Components[i] is Tdbradiogroup)or(Components[i] is TDBComboBox)or(Components[i] is TDBLookupComboBox)or(Components[i] is TDBCheckBox)or(Components[i] is TEdit))then  //verifica se os componentes são dbedits
